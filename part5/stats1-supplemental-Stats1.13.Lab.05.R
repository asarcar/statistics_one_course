...
age.n <- table1[2,2]
age.n
# Ory age.se <- table1[2,4] / sqrt(table1[2,2])
...
table2
# Ory
end.vs.age.r = table2[3,1]
end.sd = table1[4,4]
age.sd = table1[2,4]
# model1.B <- table2[3,1] * (table1[4,4] / table1[2,4])
model1.B <- end.vs.age.r * (end.sd / age.sd)
model1.B
...
# SS.X is sum of squares of X(age) == Var * Df 
SS.X <- age.sd*age.sd*(age.n - 1) 
SS.X
se.B <- sqrt( (SS.resid / df) / SS.X) 
se.B
# Validate we calculated correctly: rounding upto 4 decimal places
model1.sum <- summary(model1)
round(model1.sum$coefficients[2,2],4) == round(se.B,4)

# Print 95% confidence interval for the regression coefficient
model1.int <- confint(model1) 
model1.int
...
interval
# Validate we calculated correctly: rounding upto 4 decimal places
round(model1.int[2,],4) == round(interval,4)
